...
class Scene():
    ...
    def my_addObject(self, objMesh, rot, trans, segColor=np.array([0., 0., 1.]), useInputVars=False):
        ...
        with tf.variable_scope('obj', reuse=tf.AUTO_REUSE):
            ...
            rot = tf.get_variable(name=objID + '_rot_0', initializer=rot, dtype=tf.float32)
            self.itemPropsDict[objID].addVarToList(rot)

            trans = tf.get_variable(name=objID + '_trans_0', initializer=trans, dtype=tf.float32)
            self.itemPropsDict[objID].addVarToList(trans)
